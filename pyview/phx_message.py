from starlette.websockets import WebSocketDisconnect, Message
import json
...
def parse_message(message: Message) -> tuple[str, str, str, str, dict]:
    ...
    if "bytes" in message:      
        data = message["bytes"]
        return BinaryUploadSerDe().deserialize(data)
...
class BinaryUploadSerDe:
    def deserialize(self, encoded_data: bytes) -> tuple[str, str, str, str, dict]:
        offset = 0

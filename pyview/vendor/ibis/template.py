...
class Template:
    ...
    def tree(self, *pargs, **kwargs):
        data_dict = pargs[0] if pargs else kwargs
        strict_mode = kwargs.get("strict_mode", False)
        context = Context(data_dict, strict_mode)
        return self.root_node.tree_parts(context).render_parts()

    def tree_parts(self, *pargs, **kwargs):
        data_dict = pargs[0] if pargs else kwargs
        strict_mode = kwargs.get("strict_mode", False)
        context = Context(data_dict, strict_mode)
        return self.root_node.tree_parts(context)

    ...
    async def handle_connected(self, myJoinId, socket: LiveViewSocket):
        while True:
            ...
            if event == "allow_upload":
                allow_upload_response = socket.upload_manager.process_allow_upload(payload)
                rendered = await _render(socket)
                ...
                resp = [
                    ...
                    topic,
                    "phx_reply",
                    {"response": {"diff": rendered} | allow_upload_response, "status": "ok"},
                ]
            ...
            if event == "chunk":
                socket.upload_manager.add_chunk(joinRef, payload)

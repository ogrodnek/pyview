...
def _format_transition(transition: Transition) -> list[list[str]]:
    ...
    if isinstance(transition, str):
        ...
        return [
            [transition[0]] if isinstance(transition[0], str) else list(transition[0]),
            [transition[1]] if isinstance(transition[1], str) else list(transition[1]),
            [transition[2]] if isinstance(transition[2], str) else list(transition[2]),
        ]
    else:
        raise ValueError(
...
def _build_opts(**kwargs) -> dict[str, Any]:

...
class FlashDemoTStringLiveView(AutoEventDispatch, TemplateView, LiveView[FlashDemoContext]):
    ...
    @event
    async def save(self, event, payload, socket: LiveViewSocket[FlashDemoContext]):
    ...
    @event
    async def danger(self, event, payload, socket: LiveViewSocket[FlashDemoContext]):
    ...
    def template(self, assigns: FlashDemoContext, meta: PyViewMeta) -> Template:
        name = assigns["name"]
        flash = meta.socket.flash if meta.socket else {}

        info_banner = self.flash_banner("info", flash["info"]) if "info" in flash else ""
        error_banner = self.flash_banner("error", flash["error"]) if "error" in flash else ""
